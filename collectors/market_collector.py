...
def process_ticker_df(df: pd.DataFrame, ticker: str) -> pd.DataFrame:
    ...
    if len(df) == 1:
        logger.warning(
            f"[market] {ticker}: only 1 row in fetched window - "
            f"pct_change cannot be computed and this row will be "
            f"dropped. Caller should fetch a wider date range so a "
            f"prior trading day is available for comparison."
        )
...
def collect_market_data(
    ...
    buffer_days = max(lookback_days + 1, MIN_BUFFER_DAYS)
    start = (
        date.fromisoformat(run_date) - timedelta(days=buffer_days)
    ).isoformat()
    end = date.today().isoformat()

    keep_from = (
        date.fromisoformat(run_date) - timedelta(days=lookback_days - 1)
    ).isoformat() if lookback_days > 0 else run_date

    logger.info(
        f"[market] Collecting {len(TICKERS)} tickers | "
        f"fetch window {start} -> {end} "
        f"(keeping rows from {keep_from} onward) | lookback={lookback_days}d"
    )
    ...
    for ticker in TICKERS:
        ...
        if not df_processed.empty:
            df_processed = df_processed[
                df_processed["trade_date"] >= date.fromisoformat(keep_from)
            ]
        ...
        if not df_processed.empty:
            ...
            results[ticker] = 0
            logger.warning(
                f"[market] {ticker}: fetched data but none fell within "
                f"the intended {keep_from} -> {run_date} window"
            )
